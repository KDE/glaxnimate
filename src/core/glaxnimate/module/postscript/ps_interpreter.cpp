...
void glaxnimate::ps::Interpreter::run(const QString &name)
{
    ...
    else
    {
        ...
        for ( int i = cmd->args.size() - 1; i >= 0; i-- )
        {
            ...
            {
                error(u"Wrong value %1 passed to %2"_s.arg(args[i].to_string(), name), false);
                return;
            }
        }
...
// Stack ops
    {u"pop"_s, {{Value::Null}, [](ValueArray, Interpreter&){}}},
    {u"exch"_s, {{Value::Null, Value::Null}, [](ValueArray args, Interpreter& interpreter){
        interpreter.stack().push(std::move(args[1]));
        interpreter.stack().push(std::move(args[0]));
    }}},
    {u"dup"_s, {{}, [](ValueArray, Interpreter& interpreter){
        if ( interpreter.stack().empty() )
            interpreter.error(u"Empty stack"_s, false);
        else
            interpreter.stack().push(interpreter.stack().top());
    }}},
    {u"copy"_s, {{Value::Integer}, [](ValueArray args, Interpreter& interpreter){
    ...
    {u"index"_s, {{Value::Integer}, [](ValueArray args, Interpreter& interpreter){
    ...
    {u"roll"_s, {{Value::Integer, Value::Integer}, [](ValueArray args, Interpreter& interpreter){
        int count = args[0].cast<int>();
    ...
    {u"clear"_s, {{}, [](ValueArray, Interpreter& interpreter){
        interpreter.stack().clear();
    }}},
    {u"mark"_s, {{}, [](ValueArray, Interpreter& interpreter){
        interpreter.stack().push(Value::from<Value::Mark>());
    }}},
    {u"cleartomark"_s, {{}, [](ValueArray, Interpreter& interpreter){
    ...
    {u"counttomark"_s, {{}, [](ValueArray, Interpreter& interpreter){
        int count = 0;
...
// Math functions
    {u"add"_s, {{Value::Real, Value::Real}, [](ValueArray args, Interpreter& interpreter){
    ...
    {u"div"_s, {{Value::Real, Value::Real}, [](ValueArray args, Interpreter& interpreter){
    ...
    {u"idiv"_s, {{Value::Integer, Value::Integer}, [](ValueArray args, Interpreter& interpreter){
    ...
    {u"mod"_s, {{Value::Integer, Value::Integer}, [](ValueArray args, Interpreter& interpreter){
        auto num = args[0].cast<int>();
        auto den = args[1].cast<int>();
        if ( den == 0 )
        {
    ...
    {u"mul"_s, {{Value::Real, Value::Real}, [](ValueArray args, Interpreter& interpreter){
        if ( args[0].type() == Value::Integer && args[1].type() == args[0].type() )
            interpreter.stack().push(args[0].cast<int>() * args[1].cast<int>());
        else
            interpreter.stack().push(args[0].cast<float>() * args[1].cast<float>());
    }}},
    {u"sub"_s, {{Value::Real, Value::Real}, [](ValueArray args, Interpreter& interpreter){
        if ( args[0].type() == Value::Integer && args[1].type() == args[0].type() )
            interpreter.stack().push(args[0].cast<int>() - args[1].cast<int>());
        else
            interpreter.stack().push(args[0].cast<float>() - args[1].cast<float>());
    }}},
    {u"abs"_s, {{Value::Real}, [](ValueArray args, Interpreter& interpreter){
    ...
    {u"neg"_s, {{Value::Real}, [](ValueArray args, Interpreter& interpreter){
        if ( args[0].type() == Value::Integer )
            interpreter.stack().push(-args[0].cast<int>());
        else
            interpreter.stack().push(-args[0].cast<float>());
    }}},
    {u"ceiling"_s, {{Value::Real}, [](ValueArray args, Interpreter& interpreter){
        if ( args[0].type() == Value::Integer )
            interpreter.stack().push(args[0]);
        else
            interpreter.stack().push(qCeil(args[0].cast<float>()));
    }}},
    {u"round"_s, {{Value::Real}, [](ValueArray args, Interpreter& interpreter){
        if ( args[0].type() == Value::Integer )
            interpreter.stack().push(args[0]);
        else
            interpreter.stack().push(qRound(args[0].cast<float>()));
    }}},
    {u"floor"_s, {{Value::Real}, [](ValueArray args, Interpreter& interpreter){
        if ( args[0].type() == Value::Integer )
            interpreter.stack().push(args[0]);
        else
            interpreter.stack().push(qFloor(args[0].cast<float>()));
    }}},
    {u"truncate"_s, {{Value::Real}, [](ValueArray args, Interpreter& interpreter){
        if ( args[0].type() == Value::Integer )
        {
            interpreter.stack().push(args[0]);
        }
    ...
    {u"sqrt"_s, {{Value::Real}, [](ValueArray args, Interpreter& interpreter){
        interpreter.stack().push(math::sqrt(args[0].cast<float>()));
    }}},
    {u"atan"_s, {{Value::Real, Value::Real}, [](ValueArray args, Interpreter& interpreter){
        auto num = args[0].cast<float>();
        auto den = args[1].cast<float>();
        interpreter.stack().push(float(math::rad2deg(math::atan2(num, den))));
    }}},
    {u"cos"_s, {{Value::Real}, [](ValueArray args, Interpreter& interpreter){
        interpreter.stack().push(float(math::cos(math::deg2rad(args[0].cast<float>()))));
    }}},
    {u"sin"_s, {{Value::Real}, [](ValueArray args, Interpreter& interpreter){
        interpreter.stack().push(float(math::sin(math::deg2rad(args[0].cast<float>()))));
    }}},
    {u"exp"_s, {{Value::Real, Value::Real}, [](ValueArray args, Interpreter& interpreter){
        auto base = args[0].cast<float>();
        auto exp = args[1].cast<float>();
        interpreter.stack().push(std::pow(base, exp));
    }}},
    {u"ln"_s, {{Value::Real}, [](ValueArray args, Interpreter& interpreter){
        auto val = args[0].cast<float>();
        interpreter.stack().push(std::log(val));
    }}},
    {u"log"_s, {{Value::Real}, [](ValueArray args, Interpreter& interpreter){
        auto val = args[0].cast<float>();
        interpreter.stack().push(std::log10(val));
    }}},
    {u"rand"_s, {{}, [](ValueArray, Interpreter& interpreter){
        interpreter.stack().push(int(interpreter.memory().prng()));
    }}},
    {u"srand"_s, {{Value::Integer}, [](ValueArray args, Interpreter& interpreter){
        interpreter.memory().prng.seed(args[0].cast<int>());
    }}},
    {u"rrand"_s, {{}, [](ValueArray, Interpreter& interpreter){
        auto seed = interpreter.memory().prng();
        interpreter.memory().prng.seed(seed);
        interpreter.stack().push(int(seed));
    }}},
// Interactive
    {u"="_s, {{Value::Null}, [](ValueArray args, Interpreter& interpreter){
        interpreter.print(to_ugly_string(args[0])+ '\n');
    }}},
    {u"=="_s, {{Value::Null}, [](ValueArray args, Interpreter& interpreter){
        interpreter.print(args[0].to_pretty_string() + '\n');
    }}},
    {u"stack"_s, {{}, [](ValueArray, Interpreter& interpreter){
        for ( const auto& v : interpreter.stack() )
