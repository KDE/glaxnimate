...
void glaxnimate::io::lottie::ValidationVisitor::on_visit_document(model::Document*, model::Composition* main)
{
    ...
    if ( fixed_size.isValid() )
    {
        ...
        if ( width != fixed_size.width() )
            fmt->error(i18n("Invalid width: %1, should be %2").arg(width).arg(fixed_size.width()));

        qreal height = main->height.get();
        if ( height != fixed_size.height() )
            fmt->error(i18n("Invalid height: %1, should be %2").arg(height).arg(fixed_size.height()));
    }
    ...
    if ( !allowed_fps.empty() )
    {
        ...
        if ( std::find(allowed_fps.begin(), allowed_fps.end(), fps) == allowed_fps.end() )
        {
            ...
            fmt->error(i18n("Invalid fps: %1, should be %2").arg(fps).arg(allowed.join(" or ")));
        }
    }
    ...
    if ( max_frames > 0 )
    {
        ...
        if ( duration > max_frames )
            fmt->error(i18n("Too many frames: %1, should be less than %2").arg(duration).arg(max_frames));
    }
}
